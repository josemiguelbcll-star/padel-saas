-- 0069: fn_disponibilidad_publica v2
-- Genera slots respetando franjas_turno (duraciones dinámicas: 60 min, 90 min, etc.)
-- Si no hay franjas configuradas → fallback a duracion_turno_default (comportamiento 0068)
-- La resolución de franjas se hace inline (sin current_club_id()) porque es función pública.
...
AS $$
  ...
  -- Paso del grid = menor duración configurada para el día p_fecha
  -- Si no hay franjas, usa duracion_turno_default (= comportamiento anterior)
  paso_grid AS (
    SELECT COALESCE(
      (
        SELECT MIN(d)
  ...
  -- Grid de posibles horas de inicio: apertura → cierre, step = paso_grid
  time_grid AS (
    SELECT
  ...
  -- Para cada (cancha, hora_inicio): resolver las duraciones permitidas desde franjas_turno.
  -- Prioridad idéntica a fn_resolver_duraciones: cancha-específica > global, luego prioridad DESC, id DESC.
  -- Si ninguna franja aplica → ARRAY[duracion_turno_default].
  slot_base AS (
    SELECT
      ca.id        AS cancha_id,
  ...
  -- Expandir: un slot por cada duración válida de la franja
  -- Descarta los que se pasan del hora_cierre
  slots AS (
    SELECT
      sb.cancha_id,
      sb.cancha_nombre,
  ...
  -- Reservas activas del día (no canceladas)
  ocupados AS (
  ...
  SELECT
    ...
    s.hora_inicio,
    s.hora_fin,
    -- Un slot está ocupado si existe una reserva con el mismo inicio en esa cancha
    NOT EXISTS (
