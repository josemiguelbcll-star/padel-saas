-- ============================================================================
-- 0098_ajustes_financieros_y_cobros.sql
-- Módulo Financiero — Ajustes de Ventas Diarias, Cobro de Ingresos Manuales
-- y modificaciones de gastos/ingresos.
-- ============================================================================

-- 1. Crear o reemplazar la función para cobrar un ingreso manual pendiente
CREATE OR REPLACE FUNCTION public.fn_cobrar_otro_ingreso(
  p_ingreso_id bigint,
  p_fecha_cobro date,
...
BEGIN
  ...
  IF current_user_rol() NOT IN ('admin','vendedor') THEN
    RAISE EXCEPTION 'No tenés permisos para cobrar ingresos.';
  END IF;

  -- Obtener el ingreso y validar
  SELECT * INTO v_ingreso
  FROM otros_ingresos
  WHERE id = p_ingreso_id AND club_id = v_club_id AND activo = true;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'El ingreso no existe o no pertenece a tu club.';
  END IF;

  IF v_ingreso.fecha_cobro IS NOT NULL THEN
    RAISE EXCEPTION 'El ingreso ya se encuentra cobrado.';
  END IF;

  IF p_fecha_cobro IS NULL OR p_medio_pago IS NULL THEN
    RAISE EXCEPTION 'La fecha de cobro y el medio de pago son obligatorios.';
  END IF;
...
-- 2. Redefinir fn_obtener_resumen_financiero para que ingresos_diarios sólo considere:
--    - Canchas (alquiler, reserva_pagos usando monto_alquiler)
--    - Profesores / Escuela (clase_cobros)
--    - Buffet y Shop (ventas)
--    Excluye otros ingresos (manuales).
...
BEGIN
  ...
  -- 11. Serie de Ingresos Diarios (Sólo considera Canchas (alquiler), Escuela (profesores) y buffet/shop (ventas))
  WITH dias AS (
  ...
  pagos_dia AS (
    SELECT 
      ...
      SUM(CASE WHEN tipo = 'reembolso' THEN -monto_alquiler ELSE monto_alquiler END) AS monto
    FROM reserva_pagos
    WHERE club_id = v_club_id AND fecha_hora >= v_desde_iso AND fecha_hora <= v_hasta_iso
    GROUP BY 1
  ),
  ...
  monto_dia AS (
    SELECT 
      ...
      COALESCE(p.monto, 0) + COALESCE(c.monto, 0) + COALESCE(v.monto, 0) AS monto
    FROM dias d
