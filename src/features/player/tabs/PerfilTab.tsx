...
function ReservaCard({ r, onVerDetalle }: { r: MiReservaReal; onVerDetalle?: () => void }) {
...
function SettingsRow({ icon, label, onPress, isLast }: { icon: ReactNode; label: string; onPress?: () => void; isLast?: boolean }) {
  return (
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 16px 0' }}>
          ...
          {isLoadingReservas ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {proximas.map(r => (
                <ReservaCard key={r.id} r={r} onVerDetalle={() => setSelectedReserva(r)} />
              ))}
            </div>
          )}
        </div>
